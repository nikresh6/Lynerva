      ...
      {!bets.length ? (
        ...
        <div className="overflow-x-auto rounded-2xl border bg-surface">
          <table className="w-full min-w-[760px] text-left text-xs">
            ...
            <tbody>{bets.map((bet) => {
              const pl = profit(bet);
              return <tr key={bet.id} className="border-t"><td className="px-4 py-3 tabular">{bet.date}</td><td className="max-w-[320px] px-3 py-3 font-medium">{bet.description}</td><td className="px-3 py-3 capitalize">{bet.platform}</td><td className="px-3 py-3"><select value={bet.status} onChange={(event) => setBets((current) => current.map((item) => item.id === bet.id ? { ...item, status: event.target.value as ManualBet["status"] } : item))} className="h-8 rounded-md border bg-background px-2 text-[11px]"><option value="open">Open</option><option value="win">Win</option><option value="loss">Loss</option><option value="push">Push / void</option></select></td><td className="px-3 py-3 text-right tabular">{money(bet.stake)}</td><td className="px-3 py-3 text-right tabular">{bet.status === "win" ? <input aria-label="Payout" type="number" min="0" step="0.01" value={bet.payout || ""} onChange={(event) => setBets((current) => current.map((item) => item.id === bet.id ? { ...item, payout: Number(event.target.value || 0) } : item))} className="h-8 w-24 rounded-md border bg-background px-2 text-right text-[11px]" /> : "—"}</td><td className={cn("px-3 py-3 text-right font-semibold tabular", (pl ?? 0) > 0 ? "text-positive" : (pl ?? 0) < 0 ? "text-negative" : "")}>{money(pl)}</td><td className="px-4 py-3 text-right"><button type="button" onClick={() => setBets((current) => current.filter((item) => item.id !== bet.id))} className="grid size-7 place-items-center rounded-md text-muted hover:bg-negative-bg hover:text-negative" aria-label="Delete bet"><Trash2 size={13} /></button></td></tr>;
            })}</tbody>
          </table>
        </div>
      )}
