...
function MarketDrawer({ market, onClose }: { market: MarketOpportunity; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50" role="dialog" aria-modal="true" aria-label={hasPositiveEdge ? `Why Lynerva likes ${displayTitle}` : `Model details for ${displayTitle}`}>
      ...
      <aside className="sheet-enter scrollbar-subtle absolute inset-y-0 right-0 w-full overflow-y-auto border-l bg-surface p-5 shadow-[0_0_60px_rgb(0_0_0/0.18)] sm:max-w-[500px] sm:p-7">
        <div className="mb-6 flex items-start justify-between gap-4">
          <div><PlatformMark platform={market.platform} /><h2 className="mt-2 text-xl font-semibold leading-snug tracking-[-0.025em]">{displayTitle}</h2><p className="mt-1 text-xs text-muted">{displayMarketContext(market)}</p></div>
